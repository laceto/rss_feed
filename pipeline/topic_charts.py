"""Topic-trend visualization utilities.

Extracted from visualize_topics.py.

Generates six static charts and an optional animated GIF from topic_trends.tsv:

  A) plot_spike_heatmap    — dates x top-N topics, cell = article_count (LogNorm)
  B) plot_frequency_ts     — article_count per top-N topic over time (symlog)
  C) plot_topic_timeline   — Gantt chart: first -> last appearance per topic
  D) plot_sentiment_heatmap — dates x top-N topics, cell = sentiment_score (diverging)
  E) plot_sentiment_delta  — 7-day rolling sentiment momentum heatmap
  F) plot_signal_scatter   — spike_ratio x sentiment_score on the most recent date
  G) plot_signal_scatter_animation — animated GIF of chart F (one frame per date)

IMPORTANT — matplotlib backend:
    This module calls matplotlib.use("Agg") at import time. This is required for
    headless/CI environments where no display is available. If you need an
    interactive backend, import matplotlib and call matplotlib.use() BEFORE
    importing this module.

Module-level path constants (TOPIC_TRENDS_FILE, CHARTS_DIR) have been replaced
with explicit parameters on every function so callers can inject their own paths.

All chart functions write atomically via _save_figure() (temp file + os.replace).
Sentiment charts skip gracefully when the sentiment_score column is absent.
"""
...
import matplotlib
matplotlib.use("Agg")  # NOTE: side effect at import time — must precede pyplot import
...
def load_trends(lookback_days: int, trends_file: Path) -> pd.DataFrame:
    """Load and validate a topic_trends.tsv; apply date window and label fallback.

    Args:
        lookback_days: Number of days to look back from the most recent date in
                       the file (not from today — keeps the window relative to
                       the data, not the clock).
        trends_file:   Path to topic_trends.tsv.

    Returns:
        Filtered DataFrame with an additional 'label' column (topic_label with
        first-8-chars-of-topic_id fallback for unlabeled topics).

    Raises:
        FileNotFoundError: If trends_file does not exist.
        ValueError:        If the file is empty after loading.
    """
    if not trends_file.exists():
        raise FileNotFoundError(
            f"{trends_file} not found. "
            "Run backfill.py --phase1-only (or cluster_topics.py) first."
        )

    df = pd.read_csv(trends_file, sep="\t", parse_dates=["date"])
    if df.empty:
...
def pick_top_topics(df: pd.DataFrame, top_n: int) -> list[str]:
    ...
    Returns a topic_id list ordered by descending total article count.
    """
...
def plot_spike_heatmap(df: pd.DataFrame, top_ids: list[str], out_path: Path) -> None:
    """Heatmap: dates (rows) x topic labels (cols), colour = article_count.

    Missing combinations are shown as blank (topic not active that day).
    Colour scale uses LogNorm so outlier spikes don't wash out the palette.

    Args:
        df:       Trends DataFrame from load_trends().
        top_ids:  Topic IDs to include (from pick_top_topics()).
    ...
    ax.set_title("Topic Activity Heatmap (article count per day)", fontsize=14,
    ...
    print(f"  Saved: {out_path}")  # NOTE: uses print()
...
def plot_frequency_ts(df: pd.DataFrame, top_ids: list[str], out_path: Path) -> None:
    ...
    Y-axis uses symlog scale so a single outlier spike cannot compress all other
    lines to the baseline. Direct end-of-line labels instead of a legend box.
    ...
    # Direct end-of-line labels spread evenly across the data range.
...
def plot_topic_timeline(df: pd.DataFrame, top_ids: list[str], out_path: Path) -> None:
    ...
    Bar darkness encodes total article count (darker = more coverage overall).
    ...
    agg = agg.sort_values("first_seen")
...
def _spike_ratios_on_date(
    ...
    """Return a DataFrame of spike signals for all topics active on target_date.

    Columns: topic_id, label, spike_ratio, article_count, sentiment_score, sentiment_delta.
...
def plot_sentiment_heatmap(df: pd.DataFrame, top_ids: list[str], out_path: Path) -> None:
    """Diverging heatmap: dates x top-N topics, cell = sentiment_score.

    Red = bearish (negative), green = bullish (positive), grey = no data.
    Skipped gracefully when sentiment_score column is absent from the DataFrame.
    ...
    if not _has_sentiment(df):
        print("  Skipping sentiment heatmap — no sentiment_score data.")  # NOTE: uses print()
        return

    pivot = _build_sentiment_pivot(df, top_ids)
    if pivot.empty:
        print("  Skipping sentiment heatmap — empty pivot.")  # NOTE: uses print()
        return
...
def plot_sentiment_delta(df: pd.DataFrame, top_ids: list[str], out_path: Path) -> None:
    ...
    """
    if not _has_sentiment(df):
        print("  Skipping sentiment delta — no sentiment_score data.")  # NOTE: uses print()
        return
    ...
    if delta_pivot.empty:
        print("  Skipping sentiment delta — insufficient history for any topic.")  # NOTE: uses print()
        return
...
# ── Chart F: spike x sentiment signal scatter ─────────────────────────────────


def plot_signal_scatter(df: pd.DataFrame, out_path: Path) -> None:
    """Scatter: spike_ratio (X, log) x sentiment_score (Y) on the most recent date.
    ...
    Top-8 dots by |spike_ratio x sentiment_score| are labelled.

    Skipped gracefully when sentiment_score column is absent.

    Args:
        df:       Full trends DataFrame from load_trends() (all dates).
        out_path: Destination path for the PNG.
    """
    if not _has_sentiment(df):
        print("  Skipping signal scatter — no sentiment_score data.")  # NOTE: uses print()
        return
    ...
    if scatter_df.empty:
        print("  Skipping signal scatter — no qualifying topics on most recent date.")  # NOTE: uses print()
        return
...
def plot_signal_scatter_animation(
    ...
    """
    if not _has_sentiment(df):
        print("  Skipping scatter animation — no sentiment_score data.")  # NOTE: uses print()
        return
    ...
    if len(frames_data) < 2:
        print("  Skipping scatter animation — fewer than 2 qualifying frames.")  # NOTE: uses print()
        return

    print(f"  Building animation: {len(frames_data)} frames @ {fps} fps...")  # NOTE: uses print()
    ...
    x_split = float(all_ratios.median())
    ...
    def _draw_frame(i: int) -> None:
        ...
        if (i + 1) % 10 == 0 or i == len(frames_data) - 1:
            print(f"  Frame {i + 1}/{len(frames_data)}", flush=True)  # NOTE: uses print()
    ...
    plt.close(fig)
    print(f"  Saved: {out_path}  ({len(frames_data)} frames, {fps} fps)")  # NOTE: uses print()
